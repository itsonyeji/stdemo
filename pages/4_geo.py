...
option2 = st.selectbox('보고싶은 인구현황을 선택하세요',
                       ['구별 총인구수', '구별 총내국인수', '구별 총외국인수'])
optcols2 = 'pop' if option2 == '구별 총인구수' else \
    'korpop' if option2 == '구별 총내국인수' else 'forepop'

# optclor = px.colors.sequential.RdBu if option2 == '구별 총인구수' else \
#     px.colors.sequential.YlGn if option2 == '구별 총내국인수' else \
#         px.colors.sequential.Rainbow


seoulpop['pop3'] = seoulpop[optcols2].apply(lambda x:x/10000)

fig = go.Figure(
    go.Choroplethmapbox(geojson=geo,
                        locations=seoulpop['gu'], featureidkey='properties.name',
                        z=seoulpop['pop3'], colorscale='Viridis',
                        marker_opacity=0.7)
)
fig.update_layout(mapbox_style='open-street-map',
                  mapbox_zoom=10.5, width=800, height=600,
